...
import type { DesktopBrowserConformanceDeps, KillableProcess } from "./desktop-browser-conformance.ts";
import {
...
interface NodeDesktopBrowserConformanceDepsOptions {
  platform?: string;
  arch?: string;
...
const readMode = (mode: string | undefined): "baseline-source-build" | "release-smoke" => {
  if (mode === undefined || mode === "baseline-source-build") return "baseline-source-build";
  if (mode === "release-smoke") return "release-smoke";
...
const captureProcessStream = (stream: NodeJS.ReadableStream | null | undefined, maxChars = 4096) => {
  let text = "";
...
const daemonInfo = async (home: string) => {
  try {
...
const ensureExecutable = async (file: string, label: string) => {
  try {
...
const waitForBrowser = async (queryBrowsers: () => Promise<{ ok: boolean; output: unknown }>) => {
...
const callDaemon = async (sockPath: string, method: string, params: object, timeoutMs: number) =>
...
const daemonSocketPath = async (home: string) => {
...
const watchProcess = (child: ReturnType<typeof spawn>, name: string) =>
  new Promise<{ code: number | null; signal: NodeJS.Signals | null; name: string }>((resolve, reject) => {
...
export const createNodeDesktopBrowserConformanceDeps = ({
...
}: NodeDesktopBrowserConformanceDepsOptions = {}): DesktopBrowserConformanceDeps => ({
  platform,
  arch,
  processVersion,
  env,
  ...
  queryBrowsers: async (_bskPath, environment, timeoutMs) => {
    ...
    const browsersOutput = output as { browsers?: unknown } | null;
    const browsers = browsersOutput && Array.isArray(browsersOutput.browsers) ? browsersOutput.browsers : output;
    return { ok: true, output: browsers };
  },
  stopDaemon: async (daemon) =>
    stopChildProcess({
      child: daemon.child as KillableProcess | undefined,
  ...
  stopChrome: async (chrome) =>
    stopChildProcess({
      child: chrome.child as KillableProcess | undefined,
